...
def main():
    ...
    save_file = f'preprocessed/zinc.data'
    try:
        print('Loading saved infomation...')
        time_t = time.time()
        train_loader, valid_loader, test_loader = torch.load(save_file)
        print('Saved infomation loaded! Time:', time.time() - time_t)
    except:
        time_t = time.time()
        train_loader = []
        valid_loader = []
        ...
        print('Infomation saved! Time:', time.time() - time_t)
    ...
    average = np.mean(train_mae_list)
    standard_dev = np.std(train_mae_list)
    print('Mean train avg MAE (+- std): {} ({})'.format(average, standard_dev))
    
    average = np.mean(test_mae_list)
    standard_dev = np.std(test_mae_list)
    print(test_mae_list)
    print('Mean test avg MAE (+- std): {} ({})'.format(average, standard_dev))
